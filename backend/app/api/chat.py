...
async def _run_crew(
    ...
    async with crew_trace(session_id, user_id) as trace:
            ...
            with trace.span("memory", request.message) as span:
                try:
                    ...
                    ctx = await build_user_context(
                        ...
                        db=db, store=store, redis=redis, spotify=spotify,
                    )
        ...
        with trace.span("router", request.message) as span:
            t0 = time.monotonic()
            intent, confidence = await classify_intent(request.message, cfg)
        ...
        if intent in (IntentType.MUSIC_FIND, IntentType.MUSIC_QUEUE, IntentType.MIXED):
        ...
        if intent in (IntentType.ARTIST_INFO, IntentType.MIXED):
        ...
        if intent == IntentType.MOOD_CHECK and user_id and store:
...
async def chat(
    ...
    return StreamingResponse(
        _run_crew(request, spotify, brave, weaviate, db, redis, cfg),
